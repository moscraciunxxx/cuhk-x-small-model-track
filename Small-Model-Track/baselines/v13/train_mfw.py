"""Train MidFusePlus with GroupKFold; dump OOF + holdout logits for v13 fuse."""
...
from model import MidFusePlus, count_parameters  # noqa: E402  (v13 model)
...
def train_one(train_ds, val_ds, args, device, tag: str, val_users=None, train_users=None):
    ...
    model = MidFusePlus(num_classes=args.num_classes).to(device)
    n_params = count_parameters(model)
    print(f"[{tag}] params={n_params} model=midfuse_plus", flush=True)
...
def parse_args():
    ...
    p.add_argument("--ckpt-dir", type=str, default=str(ROOT / "checkpoints_mfw"))
    p.add_argument("--metrics-out", type=str, default=str(ROOT / "metrics_mfw.json"))
    p.add_argument("--oof-out", type=str, default=str(ROOT / "oof_mfw.npz"))
    p.add_argument("--holdout-logits-out", type=str, default=str(ROOT / "holdout_mfw.npz"))
    ...
    p.add_argument("--folds-only", type=str, default="", help="comma folds e.g. 0,1 or empty=all")
    return p.parse_args()
...
def main():
    ...
    if accs:
        ...
        # OOF acc on non-holdout only (honest)
    ...
    if not args.skip_holdout_train:
        ...
        np.savez_compressed(args.holdout_logits_out, midfuse_wide=h_logits, y=y[va].astype(np.int64), users=users[va].astype(np.int64))
        print(
            f"holdout_acc={hold['best_val_acc']:.4f} wrote {args.holdout_logits_out}",
            flush=True,
        )
        if device.type == "cuda":
            torch.cuda.empty_cache()
