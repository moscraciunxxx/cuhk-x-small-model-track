"""v13 nested late-fuse: new bases (stgcn_2s / ms2s / mfw) + v11 branches.

Overwrite only if holdout > 0.5663 AND nested OOF >= 0.558.
Selection = max nested OOF; holdout last.
"""
...
def main():
    ...
    if "ms2s" in P:
        for mix in ["Am", "Bm", "Gm", "pow_mf_ms_gru"]:
    ...
    clear_win = bool(hold_best > WIN_HOLD and oof_best >= WIN_OOF)
    ...
    eligible = {k: v for k, v in methods.items() if v["oof_acc_nested"] >= WIN_OOF}
    if eligible:
        bh = max(eligible, key=lambda k: eligible[k]["holdout_acc"])
        print(f"best hold OOF>={WIN_OOF}: {bh} hold={eligible[bh]['holdout_acc']:.4f} oof={eligible[bh]['oof_acc_nested']:.4f}", flush=True)
    ...
    if "s2s" in P:
        branches_meta["s2s"] = ("stgcn_2s", ROOT / "checkpoints_stgcn2s", "stgcn_2s")
    if "ms2s" in P:
        branches_meta["ms2s"] = ("ms2s", ROOT / "checkpoints_ms2s", "ms_stgcn_2s")
    if "mfw" in P:
    ...
    sub.to_csv(ROOT / "submission_v13_candidate.csv", index=False)
    sub.to_csv(ROOT / "submission_v13.csv", index=False)
    np.savez_compressed(ROOT / "submission_v13_probs.npz", probs=test_probs, pred=pred)
    ...
    if clear_win:
        ...
        (TRACK / "submission_README.txt").write_text(
            f"v13 clear-win AND: nested OOF {oof_best:.4f} (>=0.558) and holdout {hold_best:.4f} (>0.5663)\n"
        ...
        print("NO overwrite — track submission remains v11", flush=True)
    ...
    metrics = {
        ...
        "v11_nested_oof": V11_OOF,
        "v11_holdout": V11_HOLD,
        "clear_win": clear_win,
