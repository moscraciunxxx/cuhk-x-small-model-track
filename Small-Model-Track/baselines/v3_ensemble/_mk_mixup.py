"""Patch a copy of train.py to enable MixUp in run_epoch, then print path."""
from pathlib import Path
src = Path("train.py").read_text(encoding="utf-8")
# ensure mixup args exist
if "--mixup" not in src:
    src = src.replace(
...
# replace run_epoch body marker: inject mixup before loss = criterion
old = """            loss = criterion(logits, y)
            if train:
                loss.backward()"""
new = """            # MixUp (optional)
            mix_a = float(getattr(run_epoch, "_mixup", 0.0) or 0.0)
            if False:
                pass
            loss = criterion(logits, y)
            if train:
                loss.backward()"""
# Better: rewrite run_epoch signature usage via monkey in train_one
# Instead write a thin wrapper file.
Path("train_mixup_holdout.py").write_text('''
...
if __name__=="__main__":
    main()
''', encoding="utf-8")
print("wrote train_mixup_holdout.py")
