...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="midfuse",
        choices=["midfuse", "midfuse_wide", "deepconv", "gru_attn", "transformer", "compact", "compact_fuse"],
    )
...
def main():
    ...
    if (not args.skeleton_only) and args.model in ("midfuse", "midfuse_wide", "compact_fuse") and imu_path.exists():
        imu = np.load(imu_path, allow_pickle=False)
        X_imu = imu["X"]
        has_imu = imu["has_imu"].astype(bool)
        dual = True
        print(f"imu cache n={len(X_imu)} has={has_imu.sum()}", flush=True)
    elif args.model in ("midfuse", "midfuse_wide", "compact_fuse") and not args.skeleton_only:
        print("WARNING: midfuse requested but no imu_train.npz — falling back to deepconv", flush=True)
        args.model = "deepconv"
        dual = False
