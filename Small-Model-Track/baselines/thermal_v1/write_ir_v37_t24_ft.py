"""ir_v37: label-free ranked-prefix of native T24 FT 4ch R(2+1)D-34.
...
# Plan 5-dp floors; 1e-5 covers exact v29b te_acc 0.72955975 / 0.75483871
U8_MIN = 0.72956
U24_MIN = 0.75484
...
def load_v37():
    h = CK37 / "hold_logits.npy"
    t = CK37 / "test_logits.npy"
    if not h.exists() or not t.exists():
        raise FileNotFoundError(f"t24_ft logits missing: {h} {t}")
    hold = np.load(h).astype(np.float32)
    test = np.load(t).astype(np.float32)
    if hold.shape[1] != 40 or test.shape != (405, 40):
        raise ValueError(f"bad t24_ft shapes hold={hold.shape} test={test.shape}")
    for other in (CK42 / "hold_logits.npy", CK123 / "hold_logits.npy", CK36 / "hold_logits.npy"):
        if other.exists() and np.allclose(hold, np.load(other)):
            raise ValueError(f"t24_ft hold logits copy of {other}")
    return hold, test
...
def write_v37_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v37_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v37.csv")
    T = CFG["T"]
    kind, spec = hold["best"]["kind"], hold["best"]["spec"]
    if kind != "prefix":
        raise ValueError(f"v37 ships ranked prefix only, got {kind}")
    pmax, amin, k = spec
    ...
    diffs_v29 = [i for i, (a, b) in enumerate(zip(v29_rows, new_rows)) if a["prediction"] != b["prediction"]]
    diffs_v36 = [i for i, (a, b) in enumerate(zip(v36_rows, new_rows)) if a["prediction"] != b["prediction"]]
    file_dis = len(diffs_v29)
    file_dis_v36 = len(diffs_v36)
    only_missed = file_dis_v36 == 1 and new_rows[diffs_v36[0]]["path"] == MISSED_CLIP
    test_info = {
        "disagree_vs_v29b": int((preds != preds_v29).sum()),
        "file_dis_vs_v29b": file_dis,
        "file_dis_vs_v36": file_dis_v36,
        "diff_paths_v36": [new_rows[i]["path"] for i in diffs_v36],
    ...
    if submit_ok:
        ...
        wrote = True
    if verbose:
        print(f"test file_dis_v29b={file_dis} file_dis_v36={file_dis_v36} only_0379={only_missed} submit_ok={submit_ok} wrote={wrote}", flush=True)
...
def main():
    hold = evaluate_v37_hold(verbose=True)
    result = write_v37_submission(hold, verbose=True)
    now = datetime.now(PT).strftime("%Y-%m-%d %H:%M:%S PT")
    report = {
        "tag": "ir_v37_t24_ft_prefix",
        "updated_at": now,
        "best": hold["best"],
        ...
        "wc": 0.09,
        "note": "noirbox FT 4ch; ranked prefix; no greedy; no extra mid; no 0-diff submit",
    }
    (ROOT / "metrics_ir_v37_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
