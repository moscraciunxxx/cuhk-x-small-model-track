"""4ch T=11: subsample IR-box T16. Unused T not in v73 mix (T10/12/15/16/24)."""
...
OUT = ROOT / "cache" / "depth_ir_4ch_t11"
T11, T16, S, C = 11, 16, 112, 4
IDX = np.round(np.linspace(0, T16 - 1, T11)).astype(np.int64)


def _copy(n, src_name, dst_name):
    src = np.memmap(SRC / src_name, dtype=np.uint8, mode="r", shape=(n, T16, S, S, C))
    dst = np.memmap(OUT / dst_name, dtype=np.uint8, mode="w+", shape=(n, T11, S, S, C))
    bs = 64
...
def main():
    ...
    _copy(ntr, "train_x_t16_s112.npy", "train_x_t11_s112.npy")
    _copy(nte, "test_x_t16_s112.npy", "test_x_t11_s112.npy")
    for name in ("train_y.npy", "train_users.npy", "train_meta.json", "test_meta.json", "test_empty.json"):
        shutil.copy2(SRC / name, OUT / name)
    info = {
        "arch_input": "IR-box Depth RGB + IR gray, T=11 subsample of T16",
        "in_ch": 4, "t": T11, "size": S, "frame_idx": IDX.tolist(),
        "not_in_v73_mix": True,
        "n_train": ntr, "n_test": nte,
        "train_shape": [ntr, T11, S, S, C], "test_shape": [nte, T11, S, S, C],
    }
