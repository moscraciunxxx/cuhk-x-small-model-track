"""Gate ir_v52. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v52_ibv44_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v52_hold, write_v52_submission,
)
...
def main():
    ...
    expected = {"small_model_track_test/SM_test_0216/", "small_model_track_test/SM_test_0307/"}
    clips_ok = set(paths) == expected
    ranked = hold["best"]["kind"] == "avg_ib_v44_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    passed = (
    ...
    print(f"kind=avg_ib_v44_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
