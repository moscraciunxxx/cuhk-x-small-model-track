"""ir_v77: avg(T17 IR-box v76, T24-nonIR v44) ranked-prefix (0.15, 0.70, 2) on v73 mix.

Unused clips 0169+0283 vs v73. Nested lift. Label-free, wc=0.09.
"""
...
CK76 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v76_t17_ft"
...
def evaluate_v77_hold(verbose=True):
    ...
    v76 = np.load(CK76 / "hold_logits.npy").astype(np.float32)
    aux = aux_avg(v76, v44)
    ...
    return {
        "best": {"kind": "avg_v76_v44_on_v73", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
        ...
        "v76_test": np.load(CK76 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v77_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v77_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v77.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["v76_test"], hold["v44_test"])
...
def deploy_pack_mb():
    pack = CK76 / "model_int8.pt"
    yolo = ROOT / "yolov8n.pt"
    return ((pack.stat().st_size if pack.exists() else 0) + (yolo.stat().st_size if yolo.exists() else 0)) / (1024 * 1024)


def main():
    hold = evaluate_v77_hold(verbose=True)
    result = write_v77_submission(hold, verbose=True)
    report = {
        "tag": "ir_v77_v76v44_on_v73",
    ...
    (ROOT / "metrics_ir_v77_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
