"""ir_v38: T24 ranked-prefix k=2 stacked on the public v36 IR mix.

v36 = noirbox prefix (0.30, 0.50, 3) on v29b IR.
v38 = T24 prefix (0.30, 0.55, 2) on that IR. Label-free, wc=0.09.
Submit only if nested > v29b, users hold, and 2 <= file_dis vs v36 <= 4.
"""
...
def evaluate_v38_hold(verbose=True):
    ...
    if nb.shape != t24.shape or nb.shape[0] != hold["ir_sw"].shape[0]:
        raise ValueError(f"logit shapes nb={nb.shape} t24={t24.shape} ir={hold['ir_sw'].shape}")
    if np.allclose(nb, t24):
        raise ValueError("t24 hold logits copy of noirbox")
    ...
    ir_b, ch24 = ranked_prefix(ir_v36, t24, T, *T24_SPEC)
    ...
    if verbose:
        print(
            f"v36_n={len(ch36)} t24_n={len(ch24)} nested={nest['mean']:.6f} dis={dis} "
            f"users_ok={users_ok} clears={clears} spec_t24={T24_SPEC}",
            flush=True,
        )
...
def write_v38_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v38_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v38.csv")
    T = CFG["T"]
    ...
    ir_b, _ = ranked_prefix(ir_v36, hold["t24_test"], T, *T24_SPEC)
    ...
    only_missed = file_dis_v36 == 1 and new_rows[diffs36[0]]["path"] == MISSED_CLIP
    test_info = {
        "file_dis_vs_v36": file_dis_v36,
        "file_dis_vs_v29b": file_dis_v29,
        "diff_paths_v36": [new_rows[i]["path"] for i in diffs36],
    ...
    if verbose:
        print(
            ...
            f"only_0379={only_missed} submit_ok={submit_ok} wrote={wrote} paths={test_info['diff_paths_v36']}",
            flush=True,
        )
...
def main():
    ...
    report = {
        ...
        "wc": 0.09,
        "note": "T24 k=2 on public v36 mix; not v36 k=4; not v37 k=8",
    }
    (ROOT / "metrics_ir_v38_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
