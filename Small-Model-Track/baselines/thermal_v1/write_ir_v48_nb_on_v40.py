"""ir_v48: extra noirbox ranked-prefix (0.20, 0.60, 2) on public v40 mix. Clips 0047+0319."""
...
def evaluate_v48_hold(verbose=True):
    ...
    ir_b, ch = ranked_prefix(ir40, nb, T, *SPEC)
    ...
    return {
        "best": {"kind": "noirbox_on_v40", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
...
def write_v48_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v48_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v48.csv")
    T = CFG["T"]
    ...
    ir_b, _ = ranked_prefix(ir40, hold["nb_test"], T, *SPEC)
    ...
    keep_ok = all(next(r for r in v40_rows if r["path"] == p)["prediction"] == next(r for r in new_rows if r["path"] == p)["prediction"] for p in KEEP)
    banned_hit = [p for p in paths40 if p in BANNED]
    fd = len(diffs40)
    test_info = {"file_dis_vs_v40": fd, "file_dis_vs_v29b": len(diffs29), "diff_paths_v40": paths40, "banned_hit": banned_hit, "keep_v40_public": keep_ok}
    ...
    if submit_ok:
        ...
        wrote = True
    if verbose:
        print(f"test file_dis_v40={fd} banned={banned_hit} keep={keep_ok} submit_ok={submit_ok} wrote={wrote} paths={paths40}", flush=True)
...
def main():
    hold = evaluate_v48_hold(verbose=True)
    result = write_v48_submission(hold, verbose=True)
    report = {
        "tag": "ir_v48_nb_on_v40", "updated_at": datetime.now(PT).strftime("%Y-%m-%d %H:%M:%S PT"),
        "best": hold["best"], "clears_nested": hold["clears_nested"], "submit_ok": result.get("submit_ok"),
        "wrote_csv": result["wrote"], "csv": str(result["out_csv"]), "test": result["test"],
        "deploy_pack_mb": deploy_pack_mb(), "wc": 0.09,
    }
    (ROOT / "metrics_ir_v48_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
