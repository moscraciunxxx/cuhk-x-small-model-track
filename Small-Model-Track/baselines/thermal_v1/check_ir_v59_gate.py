"""Gate ir_v59. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v59_nbv55_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v59_hold, write_v59_submission,
)
...
def main():
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=avg_nb_v55_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    ranked = hold["best"]["kind"] == "avg_nb_v55_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    passed = (
