"""ir_v70: avg(T18 IR-box v69, T24-nonIR v44) ranked-prefix (0.15, 0.75, 4) on v66 mix.

Unused clips 0051+0077+0337 vs v66. Label-free, wc=0.09.
"""
...
CK69 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v69_t18_ft"
...
def evaluate_v70_hold(verbose=True):
    ...
    v69 = np.load(CK69 / "hold_logits.npy").astype(np.float32)
    aux = aux_avg(v69, v44)
    ...
    ir_b, ch = ranked_prefix(ir66, aux, T, *SPEC)
    ...
    return {
        "best": {"kind": "avg_v69_v44_on_v66", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
        ...
        "v69_test": np.load(CK69 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v70_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v70_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v70.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["v69_test"], hold["v44_test"])
    ir_b, _ = ranked_prefix(ir66, aux_te, T, *SPEC)
...
def main():
    ...
    (ROOT / "metrics_ir_v70_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
