"""IR v12: fuse IR(all9_base@v7) + Thermal + Mid + optional Depth-Kinetics(v12 irbox).
Write submission_ir_v12.csv ONLY if nested LOUO and holdout both beat v7 by >= +0.01
...
def main():
    ...
    # depth
    depth_dir = ROOT / "checkpoints" / "depth_yolo_r2p1d18_v12"
    depth_npz = depth_dir / "hold_logits_v12.npz"
    ...
    if has_depth:
        ...
        print("NO depth pack yet â€” will eval IR-strong / IR+Thermal diversity only if present", flush=True)
    ...
    if depth_h is not None:
        for ir_name, ir in [("all9_base", ir_v7)] + ([("ir_strong", ir_strong)] if ir_strong is not None else []):
            ...
            # 3-way IR+Depth+Mid (drop thermal) â€” diversity probe
    ...
    if not candidates:
        ...
        (ROOT / "metrics_ir_v12.json").write_text(json.dumps(report, indent=2, default=str))
    ...
    wrote = False
    out_csv = ROOT / "submission_ir_v12.csv"
    if clear:
    ...
    report = {
        ...
        "submit_recommendation": "submit_ir_v12" if clear else "leave_for_parent",
        "fp16_pack_mb": 59.85,
        "yolo_mb": 6.25,
