"""Gate ir_v79. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v79_v78v47_on_v73 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V73_CSV,
    deploy_pack_mb, evaluate_v79_hold, write_v79_submission,
)
...
def main():
    sample, v29, v73 = _load(SAMPLE_SUB), _load(V29B_CSV), _load(V73_CSV)
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=avg_v78_v47_on_v73 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    diffs = [i for i, (a, b) in enumerate(zip(v73, cand)) if a["prediction"] != b["prediction"]]
    paths = [cand[i]["path"] for i in diffs]
    banned = [p for p in paths if p in BANNED]
    keep_ok = all(a["prediction"] == b["prediction"] for a, b in zip(v73, cand) if a["path"] in KEEP)
    fd = len(diffs)
    size_mb = deploy_pack_mb()
    ranked = hold["best"]["kind"] == "avg_v78_v47_on_v73" and abs(CFG["wc"] - 0.09) < 1e-9
    expected = {"small_model_track_test/SM_test_0068/", "small_model_track_test/SM_test_0299/"}
    ...
    passed = (
        ...
        and bool(result.get("submit_ok")) and nested_acc > NEST_MIN + 1e-6
    )
    print(f"kind=avg_v78_v47_on_v73 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    print(f"nested_gt_floor={int(nested_gt)} users_ok={int(users_ok)} user_bits={users}", flush=True)
    print(f"file_dis_v73={fd} paths={paths} banned={banned} keep_public={int(keep_ok)} clips_ok={int(clips_ok)}", flush=True)
