"""Gate ir_v54. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v54_v44v53_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v54_hold, write_v54_submission,
)
...
def main():
    ...
    expected = {"small_model_track_test/SM_test_0086/", "small_model_track_test/SM_test_0390/"}
    clips_ok = set(paths) == expected
    ranked = hold["best"]["kind"] == "avg_v44_v53_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    passed = (
        nested_gt and users_ok and schema_ok and 2 <= fd <= 4 and not banned
        and keep_ok and 0 < size_mb <= 100 and clips_ok and ranked and bool(result.get("submit_ok"))
    )
    print(f"kind=avg_v44_v53_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    print(f"nested_gt_floor={int(nested_gt)} users_ok={int(users_ok)} user_bits={users}", flush=True)
    print(f"file_dis_v48={fd} paths={paths} banned={banned} keep_public={int(keep_ok)} clips_ok={int(clips_ok)}", flush=True)
