"""ir_v36: label-free ranked-prefix of non-irbox FT 4ch R(2+1)D-34.
...
def write_v36_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v36_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v36.csv")
    T = CFG["T"]
    kind, spec = hold["best"]["kind"], hold["best"]["spec"]
    if kind != "prefix":
        raise ValueError(f"v36 ships ranked prefix only, got {kind}")
    pmax, amin, k = spec
...
def main():
    hold = evaluate_v36_hold(verbose=True)
    result = write_v36_submission(hold, verbose=True)
    now = datetime.now(PT).strftime("%Y-%m-%d %H:%M:%S PT")
    report = {
        "tag": "ir_v36_noirbox_ft_prefix",
        "updated_at": now,
        "best": hold["best"],
    ...
    (ROOT / "metrics_ir_v36_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
