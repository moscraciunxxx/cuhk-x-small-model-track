"""Gate ir_v50. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v50_ibv47_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v50_hold, write_v50_submission,
)
...
def main():
    ...
    expected = {"small_model_track_test/SM_test_0024/", "small_model_track_test/SM_test_0120/"}
    clips_ok = set(paths) == expected
    ranked = hold["best"]["kind"] == "avg_ib_v47_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    passed = (
    ...
    print(f"kind=avg_ib_v47_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
