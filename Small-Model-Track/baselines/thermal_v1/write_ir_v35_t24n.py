"""ir_v35: label-free ranked-prefix of native T24 4ch R(2+1)D-34.
...
CK35 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v35_t24n"
...
def load_v35():
    """Native T24 4ch logits averaged with T16 R34 so the prefix ranking is new-view, not old T16-only."""
    h = CK35 / "hold_logits.npy"
    t = CK35 / "test_logits.npy"
    if not h.exists() or not t.exists():
        raise FileNotFoundError(f"native T24 logits missing: {h} {t}")
    hold24 = np.load(h).astype(np.float32)
    test24 = np.load(t).astype(np.float32)
    if hold24.shape[1] != 40 or test24.shape != (405, 40):
        raise ValueError(f"bad T24 shapes hold={hold24.shape} test={test24.shape}")
    up = ROOT / "checkpoints" / "depth_ir_r2p1d34_v33_t24" / "hold_logits.npy"
    for other in (CK42 / "hold_logits.npy", CK123 / "hold_logits.npy", up):
        if other.exists() and np.allclose(hold24, np.load(other)):
            raise ValueError(f"T24 hold logits copy of {other}")
    h16 = np.load(CK42 / "hold_logits.npy").astype(np.float32)
    t16 = np.load(CK42 / "test_logits.npy").astype(np.float32)
    hold = (0.5 * hold24 + 0.5 * h16).astype(np.float32)
    test = (0.5 * test24 + 0.5 * t16).astype(np.float32)
    np.save(CK35 / "hold_logits_avg_t16.npy", hold)
    np.save(CK35 / "test_logits_avg_t16.npy", test)
    return hold, test
...
def write_v35_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v35_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v35.csv")
    T = CFG["T"]
    kind, spec = hold["best"]["kind"], hold["best"]["spec"]
    if kind != "prefix":
        raise ValueError(f"v35 ships ranked prefix only, got {kind}")
    pmax, amin, k = spec
...
def main():
    hold = evaluate_v35_hold(verbose=True)
    result = write_v35_submission(hold, verbose=True)
    now = datetime.now(PT).strftime("%Y-%m-%d %H:%M:%S PT")
    report = {
        "tag": "ir_v35_t24n_prefix",
        "updated_at": now,
        "best": hold["best"],
        ...
        "wc": 0.09,
        "note": "native T24 4ch; ranked prefix; no greedy; no extra mid; no 0-diff submit",
    }
    (ROOT / "metrics_ir_v35_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
