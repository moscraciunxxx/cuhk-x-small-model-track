"""ir_v52: avg(IR-box T16, T24-nonIR v44) ranked-prefix (0.22, 0.68, 2) on v48 mix.

Test clips 0216+0307 vs v48. Unused after v50 TIE 0024+0120. Label-free, wc=0.09.
"""
...
def aux_avg(ib, v44):
    return (0.5 * ib + 0.5 * v44).astype(np.float32)


def evaluate_v52_hold(verbose=True):
    ...
    return {
        "best": {"kind": "avg_ib_v44_on_v48", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
...
def write_v52_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v52_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v52.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["ib_test"], hold["v44_test"])
...
def main():
    hold = evaluate_v52_hold(verbose=True)
    result = write_v52_submission(hold, verbose=True)
    report = {
        "tag": "ir_v52_ibv44_on_v48",
    ...
    (ROOT / "metrics_ir_v52_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
