"""R2P1D-18 + FocalCBCE on IR YOLO FT crops at T=16/24. Nested fuse later."""
...
def main():
    ...
    print(f"n={len(y)} hold={len(hold_idx)} pool={len(pool_idx)} T={args.t} cache={cache}", flush=True)
    ...
    for seed in args.seeds:
        ...
        for ep in range(1, args.epochs + 1):
            ...
            if met["acc"] > best:
                ...
                torch.save({"model": best_state, "val_acc": best, "epoch": ep, "seed": seed, "t": args.t}, ck)
    ...
    (ckpt_dir / "train_report.json").write_text(
        json.dumps({"t": args.t, "cache": str(cache), "members": {f"s{m['seed']}": m["acc"] for m in members},
