"""In-repo gate for ir_v32: label-free ranked-prefix or R34∩classic9 rejector.

Imports shipped nested_fixed / apply_cfg. Does not use greedy_safe_swap as the test rule.
wc must stay 0.09. Thresholds are the published v29b floors, not a hardcoded nested float.
"""
...
from write_ir_v32_prefix import (
    CFG, MAX_DIS, MISSED_CLIP, ROOT, U24_MIN, U8_MIN, V29B_CSV, V29B_NESTED,
    deploy_pack_mb, evaluate_v32_hold, write_v32_submission,
)

LEGAL = {"prefix", "rejector_th", "rejector_mid", "rejector_ir"}
...
def main():
    ...
    kind = hold["best"]["kind"]
    label_free = kind in LEGAL
    cfg = hold["best"]["cfg"]
    assert abs(float(cfg["wc"]) - 0.09) < 1e-9
    nest = nested_fixed(hold["ir_b"], hold["th_b"], hold["mid_b"], hold["yt"], hold["yu"], hold["mask0"], CFG)
    full, _ = apply_cfg(hold["ir_b"], hold["th_b"], hold["mid_b"], hold["yt"], hold["mask0"], CFG)
