"""Gate ir_v60. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v60_tri_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v60_hold, write_v60_submission,
)
...
def main():
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=tri_ib_v44_v55_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    ranked = hold["best"]["kind"] == "tri_ib_v44_v55_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    expected = {"small_model_track_test/SM_test_0128/", "small_model_track_test/SM_test_0313/"}
    clips_ok = set(paths) == expected
    passed = (
        nested_gt and users_ok and schema_ok and 2 <= fd <= 4 and not banned
        and not skip_hit and keep_ok and 0 < size_mb <= 100 and ranked
        and clips_ok and bool(result.get("submit_ok"))
        and nested_acc > NEST_MIN + 1e-6
    )
    print(f"kind=tri_ib_v44_v55_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    print(f"nested_gt_floor={int(nested_gt)} users_ok={int(users_ok)} user_bits={users}", flush=True)
    print(f"file_dis_v48={fd} paths={paths} banned={banned} skip={skip_hit} keep_public={int(keep_ok)} clips_ok={int(clips_ok)}", flush=True)
