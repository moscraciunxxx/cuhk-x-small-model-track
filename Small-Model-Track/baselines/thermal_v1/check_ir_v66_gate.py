"""Gate ir_v66. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v66_tri_t10_on_v48 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V48_CSV,
    deploy_pack_mb, evaluate_v66_hold, write_v66_submission,
)
...
def main():
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=tri_ib_v53_v65_on_v48 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    ranked = hold["best"]["kind"] == "tri_ib_v53_v65_on_v48" and abs(CFG["wc"] - 0.09) < 1e-9
    expected = {"small_model_track_test/SM_test_0276/", "small_model_track_test/SM_test_0298/"}
