"""ir_v60: mean(IR-box T16, T24-nonIR v44, T20 v55) ranked-prefix (0.20, 0.55, 4) on v48 mix.

Nested lift vs v48. Unused clips 0128+0313. Label-free, wc=0.09.
"""
...
from write_ir_v30_safe import (
    CK24, CK_V3, IR_AMIN, IR_MAXCH, IR_PMAX, MAX_DIS, ROOT, SAMPLE_SUB,
...
CK55 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v55_t20_ft"
...
def aux_mean(ib, v44, v55):
    return ((ib + v44 + v55) / 3).astype(np.float32)


def evaluate_v60_hold(verbose=True):
    ...
    v55 = np.load(CK55 / "hold_logits.npy").astype(np.float32)
    aux = aux_mean(ib, v44, v55)
    ...
    return {
        "best": {"kind": "tri_ib_v44_v55_on_v48", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
        ...
        "v55_test": np.load(CK55 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v60_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v60_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v60.csv")
    T = CFG["T"]
    ...
    from write_ir_v30_safe import TRACK
    ...
    aux_te = aux_mean(hold["ib_test"], hold["v44_test"], hold["v55_test"])
    ...
    if not submit_ok and out_csv.exists():
        ...
        out_csv = None
    if verbose:
        print(f"test file_dis_v48={fd} banned={banned_hit} keep={keep_ok} submit_ok={submit_ok} paths={paths}", flush=True)
...
def deploy_pack_mb():
    pack = CK55 / "model_int8.pt"
    yolo = ROOT / "yolov8n.pt"
    return ((pack.stat().st_size if pack.exists() else 0) + (yolo.stat().st_size if yolo.exists() else 0)) / (1024 * 1024)


def main():
    hold = evaluate_v60_hold(verbose=True)
    result = write_v60_submission(hold, verbose=True)
    report = {
        "tag": "ir_v60_tri_on_v48",
    ...
    (ROOT / "metrics_ir_v60_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
