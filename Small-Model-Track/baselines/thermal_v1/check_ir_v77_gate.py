"""Gate ir_v77. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v77_v76v44_on_v73 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V73_CSV,
    deploy_pack_mb, evaluate_v77_hold, write_v77_submission,
)
...
def main():
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=avg_v76_v44_on_v73 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    ranked = hold["best"]["kind"] == "avg_v76_v44_on_v73" and abs(CFG["wc"] - 0.09) < 1e-9
    expected = {"small_model_track_test/SM_test_0169/", "small_model_track_test/SM_test_0283/"}
