"""v11 Depth detect-fix: Kinetics R(2+1)D-18 on YOLO-v4 Depth_Color crops.
Stronger recipe: mixup, earlier unfreeze, longer patience. Dumps hold+test logits for fuse.
"""
...
def train_one(X, y, users, pool_idx, hold_idx, device, seed, epochs, batch_size, lr, patience,
              ckpt_path, mixup_alpha=0.2, unfreeze_ep=4):
    set_seed(seed)
    tag = f"seed{seed}"
...
def main():
    ...
    ap.add_argument("--cache-dir", default=str(ROOT / "cache" / "depth_color_yolo_v4"))
    ap.add_argument("--ckpt-dir", default=str(ROOT / "checkpoints" / "depth_yolo_r2p1d18_v11"))
    ...
    ap.add_argument("--unfreeze-ep", type=int, default=4)
    ...
    yt = y[hold_idx]
    print(f"depth members: {[(m['seed'], round(m['acc'],4)) for m in members]}", flush=True)
    print(f"depth ens hold={(hold_ens.argmax(1)==yt).mean():.4f}", flush=True)
    # also store meta keys for IR alignment
    import json
    ...
    hold_tag = "v12" if "v12" in ckpt_dir.name else ("v11" if "v11" in ckpt_dir.name else "ens"); np.savez(ckpt_dir / f"hold_logits_{hold_tag}.npz",
    ...
    (ckpt_dir / f"metrics_depth_{hold_tag}.json").write_text(json.dumps(metrics, indent=2))
