"""ir_v41: IR-box T16 seed123 ranked-prefix k=2 stacked on public v40 mix. Label-free, wc=0.09."""
...
from write_ir_v40_avg_on_v39 import AVG_SPEC, CFG, v39_ir
...
def v40_ir(ir, nb, ib, t24, T):
    ir39 = v39_ir(ir, nb, ib, T)
    avg = (0.5 * ib + 0.5 * t24).astype(np.float32)
    ir40, _ = ranked_prefix(ir39, avg, T, *AVG_SPEC)
    return ir40


def evaluate_v41_hold(verbose=True):
    ...
    s123 = np.load(CK123 / "hold_logits.npy").astype(np.float32)
    ...
    ir_b, ch = ranked_prefix(ir40, s123, T, *S123_SPEC)
    nest = nested_fixed(ir_b, th, mid, yt, yu, mask0, CFG)
    full, _ = apply_cfg(ir_b, th, mid, yt, yu * 0 + 1, CFG) if False else apply_cfg(ir_b, th, mid, yt, mask0, CFG)
    ...
    if verbose:
        print(f"s123_n={len(ch)} nested={nest['mean']:.6f} dis={dis} users_ok={users_ok} clears={clears} spec={S123_SPEC}", flush=True)
        print("user_bits", bits, flush=True)
    return {
        "best": {
            "kind": "s123_on_v40", "s123_spec": S123_SPEC, "n_s123": len(ch),
        ...
        "s123_test": np.load(CK123 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v41_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v41_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v41.csv")
    T = CFG["T"]
    ...
    ir_b, _ = ranked_prefix(ir40, hold["s123_test"], T, *S123_SPEC)
...
def main():
    ...
    report = {
        "tag": "ir_v41_s123_on_v40", "updated_at": now, "best": hold["best"],
    ...
    (ROOT / "metrics_ir_v41_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
