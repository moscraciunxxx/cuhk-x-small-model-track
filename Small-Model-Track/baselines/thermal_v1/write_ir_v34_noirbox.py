"""ir_v34: label-free ranked-prefix of non-IR-box 4ch R(2+1)D-34 (different crop).
...
CK34 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v34_noirbox"
...
def load_v34():
    h = CK34 / "hold_logits.npy"
    t = CK34 / "test_logits.npy"
    if not h.exists() or not t.exists():
        raise FileNotFoundError(f"noirbox logits missing: {h} {t}")
...
def evaluate_v34_hold(verbose=True):
    hold = evaluate_v30_hold(verbose=False)
    r34, r34_test = load_v34()
    ...
    if users_p.exists() and not np.array_equal(np.load(users_p), hold["yu"]):
        raise ValueError("noirbox hold_users order != late-fuse yu")
...
def write_v34_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v34_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v34.csv")
    T = CFG["T"]
    kind, spec = hold["best"]["kind"], hold["best"]["spec"]
    if kind != "prefix":
        raise ValueError(f"v34 ships ranked prefix only, got {kind}")
    pmax, amin, k = spec
...
def main():
    ...
    report = {
        ...
        "wc": 0.09,
        "note": "non-irbox 4ch crop; ranked prefix; no greedy; no extra mid; no 0-diff submit",
    }
    (ROOT / "metrics_ir_v34_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
