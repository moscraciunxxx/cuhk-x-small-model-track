"""ir_v66: mean(IR-box T16, T12 v53, T10 v65) ranked-prefix (0.35, 0.80, 3) on v48 mix.

Nested lift vs v48. Unused clips 0276+0298. Label-free, wc=0.09.
"""
...
def aux_mean(ib, t12, t10):
    return ((ib + t12 + t10) / 3).astype(np.float32)


def evaluate_v66_hold(verbose=True):
    ...
    return {
        "best": {"kind": "tri_ib_v53_v65_on_v48", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
...
def write_v66_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v66_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v66.csv")
    T = CFG["T"]
    ...
    aux_te = aux_mean(hold["ib_test"], hold["t12_test"], hold["t10_test"])
...
def deploy_pack_mb():
    pack = CK65 / "model_int8.pt"
    yolo = ROOT / "yolov8n.pt"
    return ((pack.stat().st_size if pack.exists() else 0) + (yolo.stat().st_size if yolo.exists() else 0)) / (1024 * 1024)


def main():
    hold = evaluate_v66_hold(verbose=True)
    result = write_v66_submission(hold, verbose=True)
    report = {
        "tag": "ir_v66_tri_t10_on_v48",
    ...
    (ROOT / "metrics_ir_v66_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
