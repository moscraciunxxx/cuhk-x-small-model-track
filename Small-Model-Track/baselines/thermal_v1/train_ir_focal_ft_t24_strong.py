"""Stronger T24 focal_ft recipe: longer train, later unfreeze, milder aug, focal retune.
Aim seed42-class solo >= 0.71. Do NOT spam weak seeds — 1-2 seeds max.
"""
...
class FocalCBCE(nn.Module):
    def __init__(self, class_weights, gamma=2.0, label_smoothing=0.01):
        super().__init__()
    ...
    def forward(self, logits, target):
        ...
        for k in WEAK:
            weak_boost = torch.where(target == k, weak_boost * 1.35, weak_boost)
        return (w * weak_boost * ((1 - p) ** self.gamma) * ce).mean()


class MildCachedClipDataset(CachedClipDataset):
    """Lower-aug variant: less aggressive crop / flip / color; no temporal reverse."""
    ...
    def __getitem__(self, i: int):
        ...
        if self.train:
            ...
            do_flip = rng.random() < 0.35  # lower flip
            if rng.random() < 0.45:  # lower crop rate; milder scale
                ch = int(h * (0.88 + 0.12 * rng.random()))
                cw = int(w * (0.88 + 0.12 * rng.random()))
            ...
            b = 0.92 + 0.16 * rng.random()
            contrast = 0.92 + 0.16 * rng.random()
...
def main():
    ...
    ap.add_argument("--init-from", type=str, default="", help="optional prior pool_seed*.pt to warm-start")
    args = ap.parse_args()
    cache = Path(args.cache_dir); ckpt_dir = Path(args.ckpt_dir)
    ...
    print(f"STRONG n={len(y)} hold={len(hold_idx)} pool={len(pool_idx)} T={args.t} "
          f"ep={args.epochs} unfreeze={args.unfreeze_ep} gamma={args.gamma} lr={args.lr} cache={cache}", flush=True)
    ...
    for seed in args.seeds:
        ...
        if args.init_from:
            init_p = Path(args.init_from)
            if init_p.exists():
                blob = torch.load(init_p, map_location="cpu", weights_only=False)
                model.load_state_dict(blob["model"], strict=False)
                print("warm-start from", init_p, "val", blob.get("val_acc"), flush=True)
        for name, p in model.named_parameters():
            if any(k in name for k in ["stem", "layer1", "layer2"]):
        ...
        for i, yi in enumerate(y[pool_idx]):
            if int(yi) in WEAK:
                w[i] *= 1.5
        tl = DataLoader(MildCachedClipDataset(X, y, users, pool_idx, train=True, seed=seed),
                        batch_size=args.batch_size,
                        sampler=WeightedRandomSampler(torch.as_tensor(w, dtype=torch.double), len(pool_idx), True),
                        num_workers=0, drop_last=True)
        vl = DataLoader(CachedClipDataset(X, y, users, hold_idx, train=False), batch_size=6, shuffle=False)
        best, best_ep, best_state = -1.0, -1, None
        for ep in range(1, args.epochs + 1):
            if ep == args.unfreeze_ep:
                for p in model.parameters():
                    p.requires_grad = True
                opt = torch.optim.AdamW(model.parameters(), lr=args.lr * 0.25, weight_decay=1e-4)
                sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=max(args.epochs - ep + 1, 1))
                print(f"[seed{seed}] unfroze@ep{ep} lr={args.lr * 0.25}", flush=True)
            ...
            print(f"[seed{seed}] ep{ep:03d} loss={loss_sum/max(n,1):.4f} tr={correct/max(n,1):.3f} "
                  f"val={met['acc']:.4f} f1={met['macro_f1']:.4f}", flush=True)
            if met["acc"] > best:
                best, best_ep = met["acc"], ep
                best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
                torch.save({"model": best_state, "val_acc": best, "epoch": ep, "seed": seed, "t": args.t,
                            "recipe": "strong_v26", "gamma": args.gamma, "unfreeze_ep": args.unfreeze_ep}, ck)
    ...
    (ckpt_dir / "train_report.json").write_text(
        json.dumps({"t": args.t, "cache": str(cache), "recipe": "strong_v26",
                    "epochs": args.epochs, "unfreeze_ep": args.unfreeze_ep, "gamma": args.gamma, "lr": args.lr,
                    "members": {f"s{m['seed']}": m["acc"] for m in members},
                    "ens": float((ens.argmax(1) == y[hold_idx]).mean())}, indent=2), encoding="utf-8")
