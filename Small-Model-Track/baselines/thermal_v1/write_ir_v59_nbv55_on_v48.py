"""ir_v59: avg(T16-nonIR v36, T20 v55) ranked-prefix (0.20, 0.75, 3) on v48 mix.

Unused vs v54 0086+0390 and v56 0032+0185. Label-free, wc=0.09.
"""
...
def aux_avg(nb, v55):
    return (0.5 * nb + 0.5 * v55).astype(np.float32)


def evaluate_v59_hold(verbose=True):
    ...
    return {
        "best": {"kind": "avg_nb_v55_on_v48", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
...
def write_v59_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v59_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v59.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["nb_test"], hold["v55_test"])
...
def main():
    hold = evaluate_v59_hold(verbose=True)
    result = write_v59_submission(hold, verbose=True)
    report = {
        "tag": "ir_v59_nbv55_on_v48",
    ...
    (ROOT / "metrics_ir_v59_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
