"""ir_v49: irbox (0.50, 0.65, 2) on v48 mix. Test clips 0222+0226 vs v48."""
...
from write_ir_v30_safe import CK24, CK_V3, IR_AMIN, IR_MAXCH, IR_PMAX, MAX_DIS, ROOT, SAMPLE_SUB, TRACK, evaluate_v30_hold, selective_swap
from write_ir_v41_s123_on_v40 import CFG, v40_ir, ranked_prefix
...
NB_SPEC, IB_SPEC = (0.20, 0.60, 2), (0.50, 0.65, 2)
BANNED = {
...
def v48_ir(ir, nb, ib, t24, T):
    ir40 = v40_ir(ir, nb, ib, t24, T)
    ir48, _ = ranked_prefix(ir40, nb, T, *NB_SPEC)
    return ir48

def evaluate_v49_hold(verbose=True):
    ...
    ir_b, ch = ranked_prefix(ir48, ib, T, *IB_SPEC)
    ...
    clears = bool(float(nest["mean"]) + 1e-12 >= NEST_MIN and users_ok and dis <= MAX_DIS)
    if verbose:
        print(f"n={len(ch)} nested={nest['mean']:.6f} dis={dis} users_ok={users_ok} clears={clears}", flush=True)
        print("user_bits", bits, flush=True)
    return {
        "best": {"kind": "irbox_on_v48", "spec": IB_SPEC, "n": len(ch), "cfg": dict(CFG),
                 "nested": float(nest["mean"]), "full": float(full), "dis": dis, "users_ok": users_ok, "users": bits, "clears": clears},
        "ir_b": ir_b, "th": th, "mid": mid, "yt": yt, "yu": yu, "mask0": mask0, "pred": pred, "nest": nest,
...
def write_v49_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v49_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v49.csv")
    T = CFG["T"]
    ...
    ir_b, _ = ranked_prefix(ir48, hold["ib_test"], T, *IB_SPEC)
    preds = (CFG["wa"] * softmax_np(ir_b, T) + CFG["wb"] * softmax_np(th_test, T) + CFG["wc"] * softmax_np(mid_test, T)).argmax(1)
    ...
    keep_ok = all(next(r for r in v48_rows if r["path"]==p)["prediction"]==next(r for r in new_rows if r["path"]==p)["prediction"] for p in KEEP)
    banned_hit = [p for p in paths if p in BANNED]
    fd = len(diffs48)
    test_info = {"file_dis_vs_v48": fd, "file_dis_vs_v29b": len(diffs29), "diff_paths_v48": paths, "banned_hit": banned_hit, "keep_v48_public": keep_ok}
    submit_ok = bool(hold["clears_nested"] and 2<=fd<=4 and len(diffs29)<=MAX_DIS and not banned_hit and keep_ok)
    wrote = False
    if submit_ok:
        shutil.copy2(out_csv, TRACK / "submission.csv"); wrote = True
    if verbose:
...
def main():
    hold = evaluate_v49_hold(verbose=True)
    result = write_v49_submission(hold, verbose=True)
    report = {"tag": "ir_v49_irbox_on_v48", "best": hold["best"], "submit_ok": result.get("submit_ok"), "test": result["test"], "deploy_pack_mb": deploy_pack_mb(), "wc": 0.09}
    (ROOT / "metrics_ir_v49_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    print(json.dumps({k: report[k] for k in ("submit_ok", "test", "deploy_pack_mb")}, indent=2, default=str), flush=True)
    return report
