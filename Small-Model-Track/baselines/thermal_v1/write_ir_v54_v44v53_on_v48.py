"""ir_v54: avg(T24-nonIR v44, T12 IR-box v53) ranked-prefix (0.15, 0.45, 3) on v48 mix.

New T=12 member solo 0.6673. Test clips 0086+0390 vs v48. Label-free, wc=0.09.
"""
...
def evaluate_v54_hold(verbose=True):
    ...
    v53 = np.load(CK53 / "hold_logits.npy").astype(np.float32)
    aux = aux_avg(v44, v53)
    ...
    return {
        ...
        "v53_test": np.load(CK53 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v54_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v54_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v54.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["v44_test"], hold["v53_test"])
...
def main():
    hold = evaluate_v54_hold(verbose=True)
    result = write_v54_submission(hold, verbose=True)
    report = {
        "tag": "ir_v54_v44v53_on_v48",
    ...
    (ROOT / "metrics_ir_v54_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
