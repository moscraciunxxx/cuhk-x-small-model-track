"""Gate ir_v73. Imports shipped nested_fixed/apply_cfg."""
...
from write_ir_v73_v72_on_v66 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V66_CSV,
    deploy_pack_mb, evaluate_v73_hold, write_v73_submission,
)
...
def main():
    ...
    if not result.get("submit_ok") or result.get("out_csv") is None:
        print(f"kind=v72_t15_on_v66 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    ...
    ranked = hold["best"]["kind"] == "v72_t15_on_v66" and abs(CFG["wc"] - 0.09) < 1e-9
    expected = {"small_model_track_test/SM_test_0012/", "small_model_track_test/SM_test_0036/"}
