"""Gate ir_v48. Imports shipped nested_fixed/apply_cfg."""
from __future__ import annotations
import csv
from pathlib import Path
from probe_ir_v24_fuse import apply_cfg, nested_fixed
from write_ir_v30_safe import SAMPLE_SUB
from write_ir_v48_nb_on_v40 import (
    BANNED, CFG, KEEP, NEST_MIN, ROOT, SPEC, U24_MIN, U8_MIN, V29B_CSV, V40_CSV,
    deploy_pack_mb, evaluate_v48_hold, write_v48_submission,
)
...
def main():
    sample, v29, v40 = _load(SAMPLE_SUB), _load(V29B_CSV), _load(V40_CSV)
    ...
    diffs = [i for i,(a,b) in enumerate(zip(v40,cand)) if a["prediction"]!=b["prediction"]]
    ...
    passed = nested_gt and users_ok and schema_ok and 2<=fd<=4 and not banned and keep_ok and 0<size_mb<=100
    print(f"kind=noirbox_on_v40 spec={SPEC} nested_acc={nested_acc:.10f}", flush=True)
    print(f"nested_gt_floor={int(nested_gt)} users_ok={int(users_ok)} user_bits={users}", flush=True)
    print(f"file_dis_v40={fd} paths={paths} banned={banned} keep_public={int(keep_ok)}", flush=True)
