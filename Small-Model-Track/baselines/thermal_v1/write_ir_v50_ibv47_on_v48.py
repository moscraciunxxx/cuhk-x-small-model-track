"""ir_v50: avg(IR-box T16, T8 IR-box) ranked-prefix (0.25, 0.45, 2) on v48 mix.

Test clips 0024+0120 vs v48. Label-free, wc=0.09.
"""
...
def aux_avg(ib, t8):
    return (0.5 * ib + 0.5 * t8).astype(np.float32)


def evaluate_v50_hold(verbose=True):
    ...
    t8 = np.load(CK47 / "hold_logits.npy").astype(np.float32)
    aux = aux_avg(ib, t8)
    ...
    return {
        "best": {"kind": "avg_ib_v47_on_v48", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
        ...
        "t8_test": np.load(CK47 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v50_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v50_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v50.csv")
    T = CFG["T"]
    ...
    aux_te = aux_avg(hold["ib_test"], hold["t8_test"])
...
def deploy_pack_mb():
    pack = CK47 / "model_int8.pt"
    yolo = ROOT / "yolov8n.pt"
    return ((pack.stat().st_size if pack.exists() else 0) + (yolo.stat().st_size if yolo.exists() else 0)) / (1024 * 1024)


def main():
    hold = evaluate_v50_hold(verbose=True)
    result = write_v50_submission(hold, verbose=True)
    report = {
        "tag": "ir_v50_ibv47_on_v48",
        ...
        "csv": str(result["out_csv"]), "test": result["test"],
        "deploy_pack_mb": deploy_pack_mb(), "wc": 0.09,
    }
    (ROOT / "metrics_ir_v50_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
