"""ir_v79: avg(T11 IR-box v78, T8 IR-box v47) ranked-prefix (0.20, 0.80, 3) on v73 mix.

Unused clips 0068+0299 vs v73. Nested lift. Label-free, wc=0.09.
"""
...
from write_ir_v73_v72_on_v66 import SPEC as V73_SPEC
...
CK47 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v47_t8_resume"
...
CK78 = ROOT / "checkpoints" / "depth_ir_r2p1d34_v78_t11_ft"
V73_CSV = ROOT / "submission_ir_v73.csv"
...
BANNED = {
    ...
    "small_model_track_test/SM_test_0337/", "small_model_track_test/SM_test_0012/",
    "small_model_track_test/SM_test_0036/",
}
KEEP = {
    ...
    "small_model_track_test/SM_test_0298/", "small_model_track_test/SM_test_0012/",
    "small_model_track_test/SM_test_0036/",
}
...
def v73_ir(ir, nb, ib, t24, t12, t10, t15, T):
    ir48 = v48_ir(ir, nb, ib, t24, T)
    ir66, _ = ranked_prefix(ir48, v66_aux(ib, t12, t10), T, *V66_SPEC)
    ir73, _ = ranked_prefix(ir66, t15, T, *V73_SPEC)
    return ir73


def evaluate_v79_hold(verbose=True):
    ...
    t15 = np.load(CK72 / "hold_logits.npy").astype(np.float32)
    v47 = np.load(CK47 / "hold_logits.npy").astype(np.float32)
    v78 = np.load(CK78 / "hold_logits.npy").astype(np.float32)
    aux = aux_avg(v78, v47)
    ...
    T = CFG["T"]
    ir73 = v73_ir(ir, nb, ib, t24, t12, t10, t15, T)
    ir_b, ch = ranked_prefix(ir73, aux, T, *SPEC)
    ...
    return {
        "best": {"kind": "avg_v78_v47_on_v73", "spec": SPEC, "n": len(ch), "cfg": dict(CFG),
        ...
        "t15_test": np.load(CK72 / "test_logits.npy").astype(np.float32),
        "v47_test": np.load(CK47 / "test_logits.npy").astype(np.float32),
        "v78_test": np.load(CK78 / "test_logits.npy").astype(np.float32),
        "clears_nested": clears, "label_free": True, "wc": CFG["wc"],
    }


def write_v79_submission(hold=None, out_csv=None, verbose=True):
    if hold is None:
        hold = evaluate_v79_hold(verbose=verbose)
    out_csv = Path(out_csv) if out_csv is not None else (ROOT / "submission_ir_v79.csv")
    T = CFG["T"]
    ...
    ir73 = v73_ir(ir_v29, hold["nb_test"], hold["ib_test"], hold["t24_test"],
                  hold["t12_test"], hold["t10_test"], hold["t15_test"], T)
    aux_te = aux_avg(hold["v78_test"], hold["v47_test"])
    ir_b, _ = ranked_prefix(ir73, aux_te, T, *SPEC)
    ...
    v73_rows = list(csv.DictReader(open(V73_CSV, encoding="utf-8")))
    v29_rows = list(csv.DictReader(open(V29B_CSV, encoding="utf-8")))
    new_rows = list(csv.DictReader(open(out_csv, encoding="utf-8")))
    diffs73 = [i for i, (a, b) in enumerate(zip(v73_rows, new_rows)) if a["prediction"] != b["prediction"]]
    diffs29 = [i for i, (a, b) in enumerate(zip(v29_rows, new_rows)) if a["prediction"] != b["prediction"]]
    paths = [new_rows[i]["path"] for i in diffs73]
    keep_ok = all(
        next(r for r in v73_rows if r["path"] == p)["prediction"]
    ...
    fd = len(diffs73)
    test_info = {
        "file_dis_vs_v73": fd, "file_dis_vs_v29b": len(diffs29),
        "diff_paths_v73": paths, "banned_hit": banned_hit, "keep_v73_public": keep_ok,
    }
    ...
    if not submit_ok and out_csv.exists():
        ...
        out_csv = None
    if verbose:
        print(f"test file_dis_v73={fd} banned={banned_hit} keep={keep_ok} submit_ok={submit_ok} paths={paths}", flush=True)
...
def deploy_pack_mb():
    pack = CK78 / "model_int8.pt"
    yolo = ROOT / "yolov8n.pt"
    return ((pack.stat().st_size if pack.exists() else 0) + (yolo.stat().st_size if yolo.exists() else 0)) / (1024 * 1024)


def main():
    hold = evaluate_v79_hold(verbose=True)
    result = write_v79_submission(hold, verbose=True)
    report = {
        "tag": "ir_v79_v78v47_on_v73",
    ...
    (ROOT / "metrics_ir_v79_status.json").write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
