"""Train small ST-GCN / ST-GCN+IMU MidFuse for CUHK-X Small Model Track v7."""
...
def train_one(train_ds, val_ds, args, device, tag: str, dual: bool, val_users=None, train_users=None):
    ...
    # strip heavy history in returned summary but keep full in file via metrics
    return {
        "tag": tag,
...
def indices_by_users(users_arr, hold_out):
    ...
    users_arr = np.asarray(users_arr)
    tr = np.where(~np.isin(users_arr, list(hold)))[0]
    va = np.where(np.isin(users_arr, list(hold)))[0]
    return tr, va
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="stgcn_fuse",
        choices=["stgcn", "stgcn_fuse", "stgcn_2s"],
    )
...
def main():
    ...
    X_imu = None
    has_imu = None
    want_dual = (not args.skeleton_only) and args.model in ("stgcn_fuse", "stgcn_2s")
    imu_path = cache / "imu_train.npz"
    if want_dual and imu_path.exists():
        X_imu, has_imu, _, _ = load_imu_caches(cache)
        dual = True
        print(f"imu cache n={len(X_imu)} has={int(has_imu.sum())}", flush=True)
    elif want_dual:
        print("WARNING: stgcn_fuse but no imu — falling back to stgcn", flush=True)
        args.model = "stgcn"
        dual = False
    ...
    if args.mode == "smoke":
        ...
        metrics["folds"] = [{k: v for k, v in result.items() if k != "history"}]
        metrics["mean_val_acc"] = result["best_val_acc"]
        metrics["std_val_acc"] = 0.0
        metrics["smoke_history_last"] = result["history"][-1] if result["history"] else None
    ...
    print(
        f"holdout={hold.get('best_val_acc')} mean_cv={metrics.get('mean_val_acc')} +/- {metrics.get('std_val_acc')}",
        flush=True,
    )
