"""Train MidFusePlus v15 (velocity + v2b branches) with GroupKFold; dump OOF/holdout."""
...
@torch.no_grad()
def predict_logits(model, ds, device, batch_size: int = 32):
...
def train_one(train_ds, val_ds, args, device, tag: str, val_users=None, train_users=None):
    ...
    model = MidFusePlus(num_classes=args.num_classes, use_velocity=not args.no_velocity).to(device)
    n_params = count_parameters(model)
    print(f"[{tag}] params={n_params} model=midfuse_plus vel={not args.no_velocity}", flush=True)
...
def parse_args():
    ...
    p.add_argument("--ckpt-dir", type=str, default=str(ROOT / "checkpoints_mfp"))
    p.add_argument("--metrics-out", type=str, default=str(ROOT / "metrics_mfp.json"))
    p.add_argument("--oof-out", type=str, default=str(ROOT / "oof_mfp.npz"))
    p.add_argument("--holdout-logits-out", type=str, default=str(ROOT / "holdout_mfp.npz"))
    ...
    p.add_argument("--folds-only", type=str, default="", help="comma folds e.g. 0 or empty=all")
    p.add_argument("--balanced-sampler", action="store_true", default=False)
...
def main():
    ...
    def make_ds(indices, train_mode: bool):
        return CachedDualDataset(
            ...
            indices,
            has_imu,
            augment=bool(use_aug and train_mode),
            seed=args.seed,
        )
    ...
    if args.folds_only.strip():
        fold_filter = set(int(x) for x in args.folds_only.split(",") if x.strip() != "")
    ...
    if accs:
        ...
        oof_acc = float((oof[nh].argmax(1) == y[nh]).mean()) if nh.any() else None
    ...
    if not args.skip_holdout_train and (fold_filter is None or len(fold_filter) > 1 or 0 not in (fold_filter or {})):
        # always allow holdout unless skip; for smoke (folds-only=0) skip holdout by default via flag
        pass
    ...
    if not args.skip_holdout_train:
        ...
        h_logits = predict_logits(hold["model"], make_ds(va, False), device, args.batch_size)
        del hold["model"]
        metrics["holdout"] = hold
        np.savez_compressed(
            args.holdout_logits_out,
            midfuse_plus=h_logits,
            y=y[va].astype(np.int64),
            users=users[va].astype(np.int64),
        )
        print(f"holdout_acc={hold['best_val_acc']:.4f} wrote {args.holdout_logits_out}", flush=True)
        if device.type == "cuda":
            torch.cuda.empty_cache()
