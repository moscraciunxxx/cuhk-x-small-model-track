"""v15 nested late-fuse: MidFusePlus (velocity) + s2s/ms2s + v11 branches.

Clear-win overwrite+ping: holdout > 0.5703 AND nested OOF >= 0.560.
...
V13_OOF = 0.5631
V13_HOLD = 0.5703
WIN_OOF_STRICT = 0.560
WIN_HOLD_STRICT = 0.5703
WIN_HOLD_ALONE = 0.580
WIN_OOF_SOFT = 0.560
WIN_HOLD_SOFT = 0.5703
...
def main():
    ...
    if "kd" in P:
        bases_spec.update(
            {
                "eq_mf_kd": (["mf_avg", "kd"], "eq"),
                "pow_mf_kd_gru": (["mf_avg", "kd", "gru_avg"], "pow"),
                "eq_mf_st_kd": (["mf_avg", "stgcn", "kd"], "eq"),
                "eq_kd_s2s": (["kd", "s2s"], "eq") if "s2s" in P else (["kd", "stgcn"], "eq"),
            }
        )
        if "s2s" in P:
            bases_spec["eq_mf_s2s_kd"] = (["mf_avg", "s2s", "kd"], "eq")
            bases_spec["eq_mf_st_s2s_kd"] = (["mf_avg", "stgcn", "s2s", "kd"], "eq")
            bases_spec["pow_mf_s2s_kd_gru"] = (["mf_avg", "s2s", "kd", "gru_avg"], "pow")
        if "ms2s" in P:
            bases_spec["eq_kd_ms"] = (["kd", "ms2s"], "eq")
            bases_spec["eq_mf_kd_ms"] = (["mf_avg", "kd", "ms2s"], "eq")
            if "s2s" in P:
                bases_spec["eq_mf_s2s_kd_ms"] = (["mf_avg", "s2s", "kd", "ms2s"], "eq")
    ...
    if "kd" in P:
        kd_mixes = [k for k in ("eq_mf_kd", "pow_mf_kd_gru", "eq_mf_st_kd", "eq_kd_s2s",
                                "eq_mf_s2s_kd", "eq_mf_st_s2s_kd", "pow_mf_s2s_kd_gru",
                                "eq_kd_ms", "eq_mf_kd_ms", "eq_mf_s2s_kd_ms") if k in nested_base]
        for mix in kd_mixes:
            for w in [0.5, 1.0, 1.5, 2.0, 3.0]:
                o = (9.0 * nested_base["v11"] + w * nested_base[mix]) / (9.0 + w)
                h = (9.0 * hold_base["v11"] + w * hold_base[mix]) / (9.0 + w)
    ...
    sub.to_csv(ROOT / "submission_v15_candidate.csv", index=False)
    sub.to_csv(ROOT / "submission_v15.csv", index=False)
    np.savez_compressed(ROOT / "submission_v15_probs.npz", probs=test_probs, pred=pred)
    ...
    if clear_win:
        ...
        (TRACK / "submission_README.txt").write_text(
            (
                f"v15 clear-win: nested OOF {oof_best:.4f} holdout {hold_best:.4f} "
                f"(gate hold>0.5703 & OOF>=0.560, or hold>=0.580)\n"
        ...
        print("NO overwrite - track submission remains v13", flush=True)
