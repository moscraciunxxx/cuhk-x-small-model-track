"""v14 nested late-fuse: new bases (stgcn_2s / ms2s / mfw) + v11 branches.

Overwrite/ping only if holdout>=0.580 OR (holdout>=0.575 and nested OOF>=0.568). Prefer holdout>0.5703 with OOF>=0.560.
...
V13_OOF = 0.5631
V13_HOLD = 0.5703
WIN_OOF_STRICT = 0.568  # with hold>=0.575
WIN_HOLD_STRICT = 0.575
...
def main():
    ...
    clear_win = bool(hold_best >= WIN_HOLD_ALONE or (hold_best >= WIN_HOLD_STRICT and oof_best >= WIN_OOF_STRICT))
    ...
    sub.to_csv(ROOT / "submission_v14_candidate.csv", index=False)
    sub.to_csv(ROOT / "submission_v14.csv", index=False)
    np.savez_compressed(ROOT / "submission_v14_probs.npz", probs=test_probs, pred=pred)
    ...
    if clear_win:
        ...
        (TRACK / "submission_README.txt").write_text(
            (
                f"v14 clear-win: nested OOF {oof_best:.4f} holdout {hold_best:.4f} "
                f"(gate hold>=0.580 or hold>=0.575&OOF>=0.568)\n"
