...
V13_OOF = 0.5631
V13_HOLD = 0.5703
WIN_OOF_STRICT = 0.660
WIN_HOLD_STRICT = 0.619
WIN_HOLD_ALONE = 0.624
WIN_OOF_SOFT = 0.660
WIN_HOLD_SOFT = 0.619
...
def main():
    ...
    candidates = [
        ...
        ("kd3", ROOT / "oof_kd3.npz", ["kd3", "kd", "student", "compact"], "kd3"),
        ("kd_mf_v13", ROOT / "oof_kd_mf_v13.npz", ["kd_mf_v13", "kd", "student", "midfuse"], "kd_mf_v13"),
        ("kd_a02", ROOT / "oof_kd_a02.npz", ["kd_a02", "kd", "student", "compact"], "kd_a02"),
    ...
    hold_files = [
        ...
        ("kd3", ROOT / "holdout_kd3.npz", ["kd3", "kd", "student", "compact"]),
        ("kd_mf_v13", ROOT / "holdout_kd_mf_v13.npz", ["kd_mf_v13", "kd", "student", "midfuse"]),
        ("kd_a02", ROOT / "holdout_kd_a02.npz", ["kd_a02", "kd", "student", "compact"]),
    ...
    if "kd" in P and "kd2" in P:
        ...
        # v17: exhaustive KD multi-blends on v16-validated pool + conf-weighted.
    ...
    for solo in ("kd", "kd2", "kd_c", "kd_alt", "kd3", "kd_mf_v13", "kd_a02", "kdv15", "mfw", "s2s", "ms2s"):
        if solo in P:
    ...
    # SELECT: max nested OOF among multi-branch blends (solos kept for ablation/peek only)
    blend_methods = {k: v for k, v in methods.items() if not str(k).startswith("solo_")}
    pool = blend_methods if blend_methods else methods
    best_name = max(pool, key=lambda k: pool[k]["oof_acc_nested"])
    ...
    if "kd3" in P:
        branches_meta["kd3"] = ("kd", ROOT / "checkpoints_kd3", "compact")
    if "kd_mf_v13" in P:
        branches_meta["kd_mf_v13"] = ("kd", ROOT / "checkpoints_kd_mf_v13", "midfuse")
    if "kd_a02" in P:
    ...
    sub.to_csv(ROOT / "submission_v17_candidate.csv", index=False)
    sub.to_csv(ROOT / "submission_v17.csv", index=False)
    np.savez_compressed(ROOT / "submission_v17_probs.npz", probs=test_probs, pred=pred)
    ...
    if clear_win:
        ...
        (TRACK / "submission_README.txt").write_text(
            (
                f"v17 clear-win: nested OOF {oof_best:.4f} holdout {hold_best:.4f} "
                f"(gate hold>=0.624, or hold>=0.619 & OOF>=0.660)\n"
        ...
        print("NO overwrite - track submission remains v16", flush=True)
    ...
    metrics = {
        ...
        "v16_nested_oof": V16_OOF, "v16_holdout": V16_HOLD, "v15_nested_oof": V15_OOF, "v15_holdout": V15_HOLD,
